  ...
  const tabs = [
    {
      ...
      label: t('nav.today'),
      icon: (active) => (
        <svg className={`w-6 h-6 ${active ? 'text-pink-500' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-center py-1 px-2 min-w-[56px] transition-all duration-200"
              >
                <div className={`relative ${isActive ? 'scale-110' : ''} transition-transform duration-200`}>
                  {tab.icon(isActive)}
                  {isActive && (
                    <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-pink-500" />
                  )}
                </div>
