...
function MoodWaveChart({ moodData, cycleLength }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H + 28}`} width="100%" style={{ display: 'block' }}>
      ...
      {phaseLabels.map((pl) => (
        <g key={pl.key}>
          <text x={pl.x} y={H + 4} textAnchor="middle" fontSize={13} fontWeight={600} fill={pl.accent} fontFamily={PMINCHO}>
            {pl.kanji}
          </text>
...
function PhaseDonut({ size = 160 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block' }}>
      ...
      <text x={cx} y={cy - 4} textAnchor="middle" fontSize={22} fontWeight={800} fill={INK} fontFamily={MARU}>
        {total}
      </text>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18, paddingBottom: 16 }}>
      ...
      {stats.moodData.length >= 2 && (
        ...
        >
          ...
          />
          <h3 style={{ fontFamily: MARU, fontSize: 17, fontWeight: 700, color: INK, marginBottom: 12, position: 'relative' }}>
            {isJa ? '気分の波' : 'Mood Wave'}
          </h3>
      ...
      >
        <h3 style={{ fontFamily: MARU, fontSize: 17, fontWeight: 700, color: INK, marginBottom: 12 }}>
